...
# equivalence prim.care.va==hosp_va.sf
...
# map the banks on top of the city’s tracts using our pal tm_shape()
...
ggplot(va.bg.tracts.utm) + 
  geom_histogram(mapping = aes(x=prim.care.20m), na.rm = TRUE) +
...
dist.near.plot <- tm_shape(va.bg.tracts.utm, unit = "mi") +
  tm_polygons(col = "prim.care.dmin", style = "jenks",palette = "Reds", 
              border.alpha = 0, title = "Distance to nearest \nhospital (m)") +
  tm_scale_bar(position = c("left", "bottom")) +
...
TSFCA <- ac(p = va.bg.tracts.utm$tpop, 
            ...
            #n = prim.care.va$BEDS, 
            D = dist.matrix, d0 = 1609*10, power=2,family = "2SFCA")
